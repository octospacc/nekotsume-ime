/*
  ...
  Modified & adapted to ネコツメIME by OctoSpacc <www.octt.eu.org>:
  |- Fixed styling bugs, suggestions up/down navigation, cleaned code a bit, + other tweaks
...
function postToBackground (data) { // handled in webextension mode by loader.js
...
function cloneElement(elmOriginal, elmClone) {
  ...
  elmClone.style.margin = styleOriginal.getPropertyValue('margin');
  elmClone.style.padding = styleOriginal.getPropertyValue('padding');
...
var postToWorker; // initialized by loader.js, used only in webpage mode
var ImeBackGroundColor_ = 'aliceblue';
var ImeID_ = 0;
var ImeCache_ = [];
var ImeCurrentDocument_ = null;
var InitDone_ = false;
var ImeEnabled_ = false;
...
function AjaxIME(doc) {
  ...
  // initializer
  {
    ...
    ImeCandidates_.style.margin = '0px';
  ...
  function ImeInsertText(text, delay) {
    var func = function(){
      ImeSetCaretPos(ImeStartPos_ + text.length);
    };
  ...
  function ImeHandleDown() {
    ...
    if (ImeSelectedIndex_ == ImeNumResults_ - 1) {
      ImeSelectedIndex_ = 0;
    } else {
      ++ImeSelectedIndex_;
    }
  ...
  function ImeHandleUp() {
    if (ImeIsHidden()) return;
    if (ImeSelectedIndex_ == 0) {
      ImeSelectedIndex_ = ImeNumResults_ - 1;
    } else {
      --ImeSelectedIndex_;
    }
    ImeHighlight();
  ...
  function ImeRequestCallback(result, source) {
    try {
      ImeClearResults();
      ...
      if (ImeResults_.length) {
        ImePreEdit_.value = ImeResults_[0];
      }
      if (ImeRawInput_ == '') {
        ImeRawInput_ = roma2hiragana(ImePreEdit_.value, false);
      }
      ImeCandidates_.style.display = 'block';
      if (ImeJsonp_) ImeJsonp_.remove();
      ImeAdd(source); // copy hiragana to list to make it selectable with right arrow key
    } catch (e) {}
  }

  function ImeShowCandidates() {
    var sourceText = ImePreEdit_.value;
    if (!ImeIsHidden()) return;
    if (ImeRawInput_ == '') ImeRawInput_ = roma2hiragana(ImePreEdit_.value, false);
    ImeCurrentDocument_ = ImeDocument_;
    (postToWorker || postToBackground)({method: 'parseNBest', text: ImeRawInput_, best: 20, source: sourceText});
  }
...
function roma2hiragana(str, delay) {
  ...
  for (var i = 0; i < text.length;) {
    ...
    while (len) {
      ...
      if (key in IMERoma2HiraganaTable_) {
        var kana = IMERoma2HiraganaTable_[key];
